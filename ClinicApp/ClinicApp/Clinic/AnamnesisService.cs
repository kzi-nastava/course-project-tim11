...
namespace ClinicApp.Clinic
{
    public class AnamnesisService
    {
        ...
        private void ViewAnamnesis(Patient patient)
        {
            ...
            if (!SystemFunctions.HealthRecords.TryGetValue(patient.UserName, out healthRecord))
            {
                Console.WriteLine("Health record with that username does not exist");
                return;
            }
            healthRecord = SystemFunctions.HealthRecords[patient.UserName];
            ...
            {
                //sort by date
                AnamnesisSortedDate(SystemFunctions.HealthRecords[patient.UserName]);
            }
            else if (user_input == "2")
            {
                //sort by doctor
                AnamnesisSortedDoctor(SystemFunctions.HealthRecords[patient.UserName]);
            }
            else if (user_input == "3")
            {
                Console.WriteLine("Enter the specific word for amnesis search:");
                string specific_word = Console.ReadLine();
                SearchAnamnesis(SystemFunctions.HealthRecords[patient.UserName],specific_word);
            }
