...
namespace ClinicApp.Clinic.Appointmens
{
    public class SuggestAppointmentService
    {
        ...
        private void SuggestAppointment(Patient patient)
        {
            ...
            bool available = AppointmentService.CheckAppointment(initial_appointment, duration, ref doctor);
            if (available)
            {
        ...
        private bool SuggestDoctorPriority(ref Doctor doctor, ref Patient patient, DateTime lastAppointment, DateTime preferredTime)
        {
            ...
            {
                bool available = AppointmentService.CheckAppointment(today, duration, ref doctor);
        ...
        private bool SuggestAppointmentPriority(ref Patient patient,DateTime preferredTime, DateTime lastAppointment)
        {
            ...
            while (preferredAppointment < lastAppointment)
            {
                foreach (Doctor doctor in UserRepository.Doctors.Values)
                {
                    ...
                    bool check = AppointmentService.CheckAppointment(preferredAppointment, duration, ref doctortmp);
                    if (check)
                    {
